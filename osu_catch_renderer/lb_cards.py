"""Compact ranked FLANK CARDS for the osu!catch results screen — the per-map
render leaderboard (owner mockup 2026-07-09), ported from the osu!STANDARD
renderer's render/lazer_results.py flank cards so catch reaches parity.
...
def bake_lb_card(entry, avatar_bytes, W_px: int, H_px: int, k: float,
    ...
    name, the Fruit/Drop/Droplet/Miss rows, Max Combo, Accuracy, mods badges,
    the big score, and the grade pill. `entry` is a leaderboard.LeaderboardEntry.
    ...
    for lbl, val, col in (("Fruit", entry.counts[0], RESULT_COLORS["FRUIT"]),
                          ("Drop", entry.counts[1], RESULT_COLORS["DROP"]),
                          ("Droplet", entry.counts[2], RESULT_COLORS["DROPLET"]),
                          ("Miss", miss, RESULT_COLORS["MISS"])):
...
def bake_board(board, W: int, H: int, title: str, *,
    ...
    `center_clear_px` overrides the cleared centre width (build_catch_board
...
def build_catch_board(cfg, meta, bm, replay_md5: str):
    """Build + bake the catch results-screen leaderboard for one render, or None.
