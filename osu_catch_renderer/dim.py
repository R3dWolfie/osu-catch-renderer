"""Background dim envelope — the std engine's DimEnvelope ported to catch
(osu-std osu_std_renderer/render/background.py, kept glide-for-glide so the
two engines fade identically; cross-engine consistency is the whole point).
...
Semantics (std §4.10, R3D preset keys bg_dim_intro/game/breaks 0-100%):
    ...
    intro dim   HELD until the FIRST object's approach begins (startTime -
                ...
                approach appears and dims underneath it (danser)
...
Catch's breaks are plain ``(start_ms, end_ms)`` tuples (beatmap._parse_breaks)
— the only signature difference from the std module, which takes pause
objects. Pure math, no GL, so the letterbox fade can reuse the same envelope
shape (build 0/0/1 → a 0..1 "in break" weight with the same glides).
"""
...
GLIDE_MS = 900.0     # std render/background.py GLIDE_MS — danser's measured
                     ...
                     # std so a std+catch render pair fades in lockstep.
...
def build_dim_envelope(intro: float, normal: float, breaks: float,
    ...
    """std build_dim_envelope with catch's (start, end) break tuples. Levels
    are fractions 0..1 (callers divide the 0-100 preset values)."""
